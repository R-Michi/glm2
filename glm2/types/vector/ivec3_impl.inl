...
{
    this->_v = _mm_loadu_si128((const __m128i*)iv);
}
...
template<typename T> inline glm2::vec<3, T, sizeof(int32_t)>& glm2::vec<3, T, sizeof(int32_t)>::load(const T* src)
{
    this->_v = _mm_loadu_si128((const __m128i*)src);
    return *this;
}
template<typename T> inline void glm2::vec<3, T, sizeof(int32_t)>::store(T* dst) const
{
    _mm_storeu_si128((__m128i*)dst, this->_v);
}
template<typename T> inline __m128i glm2::vec<3, T, sizeof(int32_t)>::intrin(void) const
{
    return this->_v;
}
